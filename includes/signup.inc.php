...
if (isset($_POST['signup-submit'])) {
  ...
  // We grab all the data which we passed from the signup form so we can use it later.  OLD
  // $username = $_POST['uid'];
  // $email = $_POST['mail'];
  // $password = $_POST['pwd'];
  ...
  // To replace with above NEW
  $username = $_POST['uid'];
  $email = $_POST['mail'];
  $firstname = $_POST['firstNUsers'];
  $lastname = $_POST['lastNUsers'];
  $cityname = $_POST['cityUsers'];
  $zipcode = $_POST['zipcodeUsers'];
  $address = $_POST['addressUsers'];
  ...
  if (empty($username) || empty($email) || empty($firstname) || empty($lastname) || empty($cityname) || empty($zipcode) || empty($address) || empty($password) || empty($passwordRepeat)) {
  // if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {           OLD
  ...
  // We check if the repeated password is NOT the same.
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCount > 0) {
        ...
        // $sql = "INSERT INTO users (uidUsers, emailUsers, pwdUsers) VALUES (?, ?, ?);";             OLD
        $sql = "INSERT INTO users (uidUsers, emailUsers, firstNUsers, lastNUsers, cityUsers, zipcodeUsers, addressUsers, pwdUsers) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          // Next we need to bind the type of parameters we expect to pass into the statement, and bind the data from the user.
          // mysqli_stmt_bind_param($stmt, "sss", $username, $email, $hashedPwd);       OLD
          mysqli_stmt_bind_param($stmt, "ssssssss", $username, $email, $firstname, $lastname, $cityname, $zipcode, $address, $hashedPwd);
          // Then we execute the prepared statement and send it to the database!
